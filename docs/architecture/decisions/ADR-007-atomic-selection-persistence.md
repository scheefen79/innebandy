...
- En snäv PostgreSQL-funktion sparar en komplett automatisk ordinarie fördelning i en enda transaktion. Endast Next.js-serverns server-only Supabase-klient får anropa funktionen; webbläsarens `authenticated`-roll saknar `EXECUTE`.
- Route-handlern verifierar den inloggade användaren med den vanliga sessionsklienten, räknar om fördelningen med en serverbestämd aktuell tidsgräns och skickar användar-id:t till funktionen. Funktionen accepterar endast `service_role` och verifierar att detta användar-id har aktivt medlemskap i laget.
- `SUPABASE_SERVICE_ROLE_KEY` får endast finnas som servermiljövariabel. Den får aldrig exponeras med prefixet `NEXT_PUBLIC_`, importeras i klientkod eller lagras i Git.
...
- Servernyckeln har bred databasbehörighet. Därför isoleras den i en server-only modul och används bara för den avgränsade persistensfunktionen efter separat sessions- och medlemskontroll.
