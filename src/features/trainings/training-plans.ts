...
const statuses=new Set<TrainingStatus>(["draft","planned","completed","cancelled"]);
const editableStatuses=new Set<TrainingStatus>(["draft","planned","completed"]);
...
export function validateTrainingPayload(raw:{focus:string;keyMessage:string;coachNotes:string;status:string;items:string}){
 ...
 if(!raw.focus.trim()||raw.focus.length>500||!raw.keyMessage.trim()||raw.keyMessage.length>300||raw.coachNotes.length>5000||!editableStatuses.has(raw.status as TrainingStatus)||!Array.isArray(items)||items.length>50)return {ok:false as const,error:"Kontrollera träningsplanens innehåll."};
 for(const value of items){if(!value||typeof value!=="object")return {ok:false as const,error:"En övning är ogiltig."};const item=value as Record<string,unknown>;const points=item.coachingPoints;const guide=item.guideMinutes;const validUrl=(candidate:unknown)=>candidate==null||candidate===""||(typeof candidate==="string"&&candidate.length<=1000&&candidate.startsWith("https://"));if(!sections.has(item.section as TrainingSection)||!text(item.title).trim()||text(item.title).length>200||text(item.sourceTitle).length>200||!Array.isArray(points)||points.length>20||points.some(point=>typeof point!=="string"||!point.trim()||point.length>300)||text(item.purpose).length>2000||text(item.instructions).length>5000||(guide!=null&&guide!==""&&(!Number.isInteger(guide)||Number(guide)<1||Number(guide)>120))||!validUrl(item.sourceUrl)||!validUrl(item.sourceImageUrl))return {ok:false as const,error:"Kontrollera övningarnas innehåll, tider och källor."};}
...
export function canonicalizeSourceItemTitles(items:unknown[]){
 return items.map(value=>{
  if(!value||typeof value!=="object")return value;
  const item=value as Record<string,unknown>;
  const sourceUrl=nullableText(item.sourceUrl),sourceTitle=nullableText(item.sourceTitle);
  return sourceTitle&&/^https:\/\/(www\.)?innebandy\.se\/ovningsbanken\//.test(sourceUrl??"")?{...item,title:sourceTitle}:item;
 });
}
...
export async function saveTrainingPlanSeries(admin:SupabaseClient,input:{actorUserId:string;teamId:string;seasonId:string;trainingId:string;revision:number;requestId:string;focus:string;keyMessage:string;coachNotes:string;status:TrainingStatus;items:unknown[]}){
 ...
 if(error.message.includes("TRAINING_COMPLETED")||error.message.includes("TRAINING_CANCELLED")||error.message.includes("INVALID_TRAINING"))return {result:"invalid" as const};
