...
describe("overview",()=>{
 ...
 it("loads the atomic RPC envelope",async()=>{const payload={seasonName:"Säsong",serverNow:"2026-08-24T12:00:00Z",upcomingMatches:[],players:[]};const rpc=vi.fn().mockResolvedValue({data:payload,error:null});await expect(loadOverview({rpc} as unknown as SupabaseClient,"team")).resolves.toEqual(payload);expect(rpc).toHaveBeenCalledWith("get_overview",{target_team_id:"team"});});
});
