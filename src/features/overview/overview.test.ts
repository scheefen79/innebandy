...
describe("overview",()=>{
 ...
 it("loads the atomic home envelope including next training",async()=>{const payload={seasonName:"Säsong",serverNow:"2026-08-24T12:00:00Z",nextTraining:{id:"t1",startsAt:"2026-09-05T08:00:00Z",endsAt:"2026-09-05T09:00:00Z",themeBlock:1,focus:"Passningar",keyMessage:"PASSA",status:"draft"},upcomingMatches:[],players:[]};const rpc=vi.fn().mockResolvedValue({data:payload,error:null});await expect(loadOverview({rpc} as unknown as SupabaseClient)).resolves.toEqual(payload);expect(rpc).toHaveBeenCalledWith("get_home_overview");});
});
