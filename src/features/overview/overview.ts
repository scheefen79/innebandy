...
export type OverviewData = { seasonName:string; serverNow:string; nextTraining:OverviewTraining|null; upcomingMatches:OverviewMatch[]; players:OverviewPlayer[] };
...
export async function loadOverview(supabase:SupabaseClient):Promise<OverviewData|null>{
 ...
 return {seasonName:envelope.seasonName,serverNow:envelope.serverNow,nextTraining:training,upcomingMatches:matches,players};
}
