  ...
  return <form action={`/trainings/${training.id}/save`} method="post" className="mt-6 space-y-6">
    ...
    <fieldset><legend className="text-xl font-semibold">Moment</legend><p className="mt-1 text-sm text-slate-600">Minuterna är endast en guide.</p><div className="mt-4 space-y-4">
      {items.map((item, index) => <section key={item.id} className="rounded-2xl border border-slate-200 bg-white p-4">
        ...
        {item.sourceTitle && /^https:\/\/(www\.)?innebandy\.se\/ovningsbanken\//.test(item.sourceUrl ?? "") ? <p className="mt-2 text-sm"><span className="font-semibold">Övningsnamn</span><br />{item.sourceTitle}</p> : <label className="mt-2 block text-sm font-semibold">Rubrik<input value={item.title} onChange={event => update(index, { title: event.target.value })} className="mt-1 min-h-11 w-full rounded-lg border border-slate-300 px-3" /></label>}
