...
import { additionCandidates, isRecommendedForBlock, replacementCandidates, type ExerciseCatalogItem } from "@/features/trainings/exercise-catalog";
...
function RecommendationLabel({ exercise, themeBlock }: { exercise: ExerciseCatalogItem; themeBlock: number }) {
  return isRecommendedForBlock(exercise, themeBlock) ? <span className="mt-2 inline-flex rounded-full bg-blue-100 px-2 py-1 text-xs font-bold text-blue-900">Rekommenderad för block {themeBlock}</span> : <span className="mt-2 inline-flex rounded-full bg-slate-100 px-2 py-1 text-xs font-semibold text-slate-700">Valbar utanför blockets fokus</span>;
}
  ...
  return <>
    ...
    {open ? <div role="dialog" aria-modal="true" aria-label={`Byt övning: ${item.title}`} className="fixed inset-0 z-50 overflow-y-auto bg-slate-950/60 p-4 sm:p-8">
      <div className="mx-auto max-w-2xl rounded-2xl bg-white p-5 shadow-xl sm:p-6">
        <div className="flex items-start justify-between gap-4"><div><h2 className="text-xl font-bold text-slate-950">Byt övning</h2><p className="mt-1 text-sm text-slate-600">Alla verifierade alternativ för momenttypen visas. Rekommenderade övningar är märkta för block {themeBlock}.</p></div><button type="button" onClick={close} className="min-h-11 px-2 text-sm font-semibold text-slate-700" aria-label="Stäng ersättningsvyn">Stäng</button></div>
        {candidates.length === 0 ? <div className="mt-5 rounded-xl bg-slate-50 p-4"><h3 className="font-semibold text-slate-950">Inga verifierade alternativ ännu</h3><p className="mt-1 text-sm text-slate-600">Du kan fortfarande redigera momentet manuellt.</p></div> : <ul className="mt-5 space-y-3">{candidates.map(candidate => <li key={candidate.id} className="rounded-xl border border-slate-200 p-4"><div className="flex flex-wrap items-start justify-between gap-3"><div><h3 className="font-semibold text-slate-950">{candidate.title}</h3><p className="mt-1 text-xs font-semibold text-blue-800">{candidate.themes.join(" · ")}</p><RecommendationLabel exercise={candidate} themeBlock={themeBlock} /></div><button type="button" onClick={() => selectCandidate(candidate)} className="min-h-11 rounded-lg border border-blue-700 px-3 text-sm font-semibold text-blue-700">{selected?.id === candidate.id ? "Dölj" : "Visa övning"}</button></div>{selected?.id === candidate.id ? <Preview exercise={candidate} /> : null}</li>)}</ul>}
...
export function ExerciseAdditionPicker({ themeBlock, onAdd, onCreateManual }: { themeBlock: number; onAdd: (exercise: ExerciseCatalogItem, section: TrainingSection) => void; onCreateManual: () => void }) {
  ...
  return <>
    ...
    {open ? <div role="dialog" aria-modal="true" aria-label="Lägg till övning" className="fixed inset-0 z-50 overflow-y-auto bg-slate-950/60 p-4 sm:p-8">
      <div className="mx-auto max-w-2xl rounded-2xl bg-white p-5 shadow-xl sm:p-6">
        <div className="flex items-start justify-between gap-4"><div><h2 className="text-xl font-bold text-slate-950">Lägg till övning</h2><p className="mt-1 text-sm text-slate-600">Välj valfri verifierad övning eller skapa ett eget moment. Rekommenderade övningar är märkta för block {themeBlock}.</p></div><button type="button" onClick={close} className="min-h-11 px-2 text-sm font-semibold text-slate-700">Stäng</button></div>
