...
export async function POST(request: NextRequest) {
  ...
  const context=await loadTeamContext(supabase); if(!context||context.role!=="coach")return go("/access-denied"); const form=await request.formData(); const validated=validatePlayerInput(Object.fromEntries(["first_name","last_name","level"].map(key=>[key,String(form.get(key)??"")]))); if(!validated.ok)return go(`/players/new?error=${encodeURIComponent(validated.error)}`);
