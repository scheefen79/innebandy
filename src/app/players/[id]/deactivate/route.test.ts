...
vi.mock("@/lib/supabase/route-handler",()=>({createRouteHandlerClient:()=>({applyAuthState:(r:Response)=>r,supabase:{auth:{getClaims:async()=>({data:{claims:{sub:"actor"}}})}}})}));vi.mock("@/lib/auth/team-context",()=>({loadTeamContext:async()=>({teamId:"team",seasonId:"season"})}));vi.mock("@/lib/supabase/admin",()=>({createAdminClient:()=>({})}));vi.mock("@/features/players/player-management",()=>({mutatePlayer:mutate}));
