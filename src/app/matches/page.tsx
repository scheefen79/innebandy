...
export default async function MatchesPage({ searchParams }: { searchParams: Promise<{ view?: string }> }) {
  ...
  const filter = (await searchParams).view === "all" ? "all" : "upcoming";
  ...
  return <AppShell currentItem="Matcher">
    <div className="mx-auto max-w-3xl">
      <div className="flex flex-wrap items-start justify-between gap-4">
        ...
        <Link href="/matches/new" className="flex min-h-11 items-center rounded-xl bg-blue-700 px-4 py-2 text-sm font-semibold text-white hover:bg-blue-800 focus-visible:outline-2 focus-visible:outline-offset-2 focus-visible:outline-blue-700">Ny match</Link>
      </div>
