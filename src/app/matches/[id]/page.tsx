...
export default async function MatchPage({ params, searchParams }: { params: Promise<{ id: string }>; searchParams: Promise<{ tab?: string; adjustment?: string }> }) {
  ...
  const visiblePlayers = roster.filter((player) => tab === "team" ? player.selected : !player.selected && (player.isActive || player.selectionStatus === "removed"));
