...
export default async function MatchPage({ params, searchParams }: { params: Promise<{ id: string }>; searchParams: Promise<{ tab?: string; adjustment?: string; extra?: string; completion?: string }> }) {
  ...
  const fingerprint = context.role === "coach" && canAdjust ? await loadManualAdjustmentFingerprint(supabase, context.teamId, context.seasonId, id) : null;
  const extraSource = context.role === "coach" && shouldLoadExtraSubstituteSource({ canAdjust, hasSavedRegularRoster, extraPlayerCount: extraPlayers.length })
  ...
  return <AppShell currentItem="Matcher" role={context.role}><article className="mx-auto max-w-2xl">
