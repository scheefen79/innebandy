...
export async function POST(request: NextRequest, { params }: { params: Promise<{ id: string }> }) {
  ...
  if (![id, outgoingPlayerId, incomingPlayerId].every(isUuid) || !/^[a-f0-9]{32}$/.test(fingerprint)) return redirectTo(`/matches/${id}?adjustment=invalid`);
  const result = await mutateManualAdjustment(createAdminClient(), "restore_manual_regular_adjustment", {
  ...
  return redirectTo(`/matches/${id}?adjustment=${result === "ok" ? "restored" : result}`);
}
