...
it("removes an extra substitute through the server-only boundary", async () => {
  const matchId = "c5000000-0000-4000-8000-000000000001";
  const playerId = "c4000000-0000-4000-8000-000000000002";
  const response = await POST(new NextRequest(`https://app.example/matches/${matchId}/extras/remove`, { method: "POST", body: new URLSearchParams({ playerId, fingerprint: "a".repeat(32) }) }), { params: Promise.resolve({ id: matchId }) });
  expect(mutate).toHaveBeenCalledWith({ kind: "admin" }, "remove_extra_substitute", expect.objectContaining({ matchId, playerId }));
  expect(response.status).toBe(303);
  expect(response.headers.get("location")).toBe(`https://app.example/matches/${matchId}?extra=removed`);
});
