...
import { changeTeamMemberRole } from "@/features/team-members/team-member-management";
...
export async function POST(request: NextRequest, { params }: { params: Promise<{ userId: string }> }) {
  ...
  if (!isUuid(targetUserId) || (role !== "coach" && role !== "viewer") || !/^[a-f0-9]{32}$/.test(fingerprint)) {
    return go("/team?mutationError=invalid");
  }

  const result = await changeTeamMemberRole(createAdminClient(), { actorUserId: userId, teamId: context.teamId, targetUserId, role, fingerprint });
  if (result !== "ok") return go(`/team?mutationError=${result}`);
  return go("/team?change=role_updated");
}
