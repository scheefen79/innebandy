...
import { reactivateTeamMember } from "@/features/team-members/team-member-management";
...
export async function POST(request: NextRequest, { params }: { params: Promise<{ userId: string }> }) {
  ...
  const result = await reactivateTeamMember(createAdminClient(), { actorUserId: userId, teamId: context.teamId, targetUserId, fingerprint });
  if (result !== "ok") return go(`/team?mutationError=${result}`);
  return go("/team?change=reactivated");
}
