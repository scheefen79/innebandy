...
describe("AppShell navigation",()=>{
 it("links all primary views for a coach and marks the current page",()=>{const html=renderToStaticMarkup(<AppShell currentItem="Träningar" role="coach"><p>Content</p></AppShell>);expect(html).toContain('href="/"');expect(html).toContain('href="/matches"');expect(html).toContain('href="/players"');expect(html).toContain('href="/trainings"');expect(html).toContain('href="/training-attendance"');expect(html).toContain('href="/team"');expect(html).toMatch(/aria-current="page"[^>]+href="\/trainings"/);});
