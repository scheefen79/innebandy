...
select results_eq($$select save_training_plan_series('a1000000-0000-4000-8000-000000000001','a2000000-0000-4000-8000-000000000001','a3000000-0000-4000-8000-000000000001','a4000000-0000-4000-8000-00000000000a',1,'series-1','Nytt fokus','NYTT','Anteckning','planned','[{"section":"technique","title":"Ny övning","coachingPoints":["Titta upp"]},{"section":"closing","title":"Avslut","coachingPoints":[]}]')$$,array[1],'series save reports one updated sibling');
...
select results_eq($$select count(*) from training_items where title='Gammal övning'$$,array[5::bigint],'only the edited session and its sibling had items replaced');

-- Idempotens och konflikter.
select results_eq($$select save_training_plan_series('a1000000-0000-4000-8000-000000000001','a2000000-0000-4000-8000-000000000001','a3000000-0000-4000-8000-000000000001','a4000000-0000-4000-8000-00000000000a',1,'series-1','Nytt fokus','NYTT','Anteckning','planned','[{"section":"technique","title":"Ny övning","coachingPoints":["Titta upp"]},{"section":"closing","title":"Avslut","coachingPoints":[]}]')$$,array[1],'exact retry is idempotent');
